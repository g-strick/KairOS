...
@/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/agent-contracts.md
@/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/context-budget.md
@/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/gates.md
</required_reading>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/planner-antipatterns.md
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       @/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/checkpoints.md
       @/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/tdd.md
       @/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/executor-examples.md'}
       </execution_context>
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/references/execute-mvp-tdd.md`): when `MVP_MODE=true` AND `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit), the end-of-phase TDD review escalates from advisory to **blocking under MVP+TDD** — refuse to mark the phase complete and present:
...
Read and follow `/Users/graysonstricker/Programs/KairOS/.claude/gsd-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
